...
__hot fpta_shove_t fpta_shove_name(const char *name,
  ...
  fpta_shove_t shove = t1ha1_le(uppercase, i, type) << fpta_name_hash_shift;
  if (type == fpta_table)
...
static bool fpta_schema_validate(const fpta_shove_t schema_key,
  ...
  uint64_t checksum =
      t1ha1_le(&schema->signature, schema_data.iov_len - sizeof(checksum),
...
int fpta_table_create(fpta_txn *txn, const char *table_name,
  ...
  if (rc == MDBX_SUCCESS) {
    ...
    record->checksum =
        t1ha1_le(&record->signature, bytes - sizeof(record->checksum),
                 FTPA_SCHEMA_CHECKSEED);
