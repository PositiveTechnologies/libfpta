 ...
 *
 * Аргумент file_mode задаёт маску прав доступа, которая используются
 * в случае создания новой БД. Значение 0 означает требование открыть
 * существующую БД, с возвратом ошибки при её отсутствии.
 *
 * Аргумент megabytes задаёт фиксированный размер БД в мегабайтах. При создании
 * новой БД за основным файлом БД резервируется указанное место. При открытии
 ...
 *
 * ВАЖНО: Задаваемый аргументом megabytes размер в дальнейшем НЕ БУДЕТ
 * автоматически увеличиваться или уменьшаться, в том числе если ранее БД была
 * создана с параметрами предполагающими автоматическое управление размером БД.
 * Если требуется иное поведение, то следует использовать функции
 * fpta_db_open_existing() или fpta_db_create_or_open().
 *
...
typedef struct fpta_table_stat {
  ...
  size_t
      ...
      ;
  unsigned cost_scan_O1N /* Амортизационная условная стоимость последовательного
                          * перебора в перерасчете на одну запись, иначе говоря
                          * стоимость одного шага перебора. */
      ;
  unsigned cost_search_OlogN /* Амортизационная условная стоимость поиска записи
                              * с использованием индекса; следует учитывать,
                              * что доступ к записи по вторичному индексу
                              * требует также поиска в первичном. */
      ;
  unsigned cost_uniq_MOlogN /* Амортизационная услованя стоимость провеки
                             * уникальности вставляемых или обновляемых значений
                             * полей. */
      ;
  unsigned
      cost_alter_MOlogN /* Амортизационная условная стоимость
